...
RSpec.describe "/items", type: :request do
  ...
  let(:valid_attributes) { { user_id: User.create(email: 'test@test.test').id, part_id: Nozzle.first.id, part_type: 'Nozzle' }
  }

  let(:invalid_attributes) { { pouet: 'jaune' }
  }
  ...
  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Item" do
        expect { post items_url, params: { item: valid_attributes }, headers: valid_headers, as: :json
        }.to change(Item, :count).by(1)
      end

      it "renders a JSON response with the new item" do
        post items_url, params: { item: valid_attributes }, headers: valid_headers, as: :json
    ...
    context "with invalid parameters" do
      it "does not create a new Item" do
        expect { post items_url, params: { item: invalid_attributes }, as: :json
        }.to change(Item, :count).by(0)
      end

      it "renders a JSON response with errors for the new item" do
        post items_url, params: { item: invalid_attributes }, headers: valid_headers, as: :json
  ...
  describe "DELETE /destroy" do
    it "destroys the requested item" do
      item = Item.create! valid_attributes
      expect { delete item_url(item), headers: valid_headers, as: :json
      }.to change(Item, :count).by(-1)
    end
  end
end
